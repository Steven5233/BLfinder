"""
BLFinder v3.1 — core/scanner.py
Main Scanner — Full Evidence Capture Integration

Every _check_* module now:
  1. Records baseline + attack HTTP pairs via EvidenceCapture
  2. Builds an EvidencePackage with real diff + impact analysis
  3. Attaches the package to the Finding as f.evidence_package
  4. Uses pkg.summary as the Finding.evidence field (real data, no placeholders)

Phase 1 integrations retained:
  - BlindIDORScanner (5-oracle blind IDOR detection)
  - SessionManager (token refresh + CSRF extraction)
  - FlowReplayer + FlowTemplates (multi-step flow attacks)
  - SemanticDiff (JSON-aware response comparison)
  - OAuthHandler (OAuth2 vulnerability testing)
"""
...
class BLFScanner:
    """
    BLFinder v3.1 — Business Logic Flaw Scanner
    Full evidence capture integration.
    """
    ...
    def __init__(self, config: ScanConfig):
        ...
        # Phase 1 components (initialised in __aenter__)
    ...
    async def _request(
        ...
        **kwargs,
        ...
        """
        Core request method — rate limiting, session management,
        CSRF injection, automatic 401 refresh.
        """
    ...
    def _new_evidence(self) -> "EvidenceCapture | None":
        """Return a fresh EvidenceCapture if the evidence module is available."""
    ...
    async def _req_ev(
        ...
        **kwargs,
        ...
        """
        Wrapper: calls _request() and records the exchange into EvidenceCapture.
        Use this instead of _request() inside every _check_* module for the
        baseline and attack requests so evidence is captured automatically.
        """
        ...
        if ev is not None:
            built_headers = self._build_headers(extra_headers, token=token_override)
            ev.record(
                label=label,
                method=method,
                url=url,
                req_headers=built_headers,
                req_body=req_body,
                resp_status=status,
                resp_headers=resp_hdrs,
                resp_body=resp_body,
                elapsed=elapsed,
            )
    ...
    def _build_pkg(
        ...
        """Build an EvidencePackage from a capture session."""
    ...
    def _attach(self, f: Finding, pkg: "EvidencePackage | None") -> Finding:
        """
        Attach an EvidencePackage to a Finding.
        Also overwrites f.evidence with pkg.summary (real data).
        """
        if pkg is not None:
    ...
    def _mutate_nested(
        ...
        if isinstance(obj, dict):
            ...
            for i, item in enumerate(obj):
                for sub in self._mutate_nested(
                    item, target_key, new_val, depth + 1
                ):
                    m = copy.deepcopy(obj)
                    m[i] = sub
                    mutations.append(m)
    ...
    async def run_all_modules(self, endpoints: list[dict]) -> list[Finding]:
        print(f"[*] BLFinder v3.1 — Target: {self.config.target_url}")
        ...
        above   = [f for f in raw if getattr(f, "confidence", 0) >= self.config.min_confidence]
        ...
        for field_path, key, original in price_fields:
            for tampered in tamper_values:
                for mutated_body in self._mutate_nested(body, key, tampered):
                    ...
                    # Baseline
                    await self._req_ev(
                        "baseline", ev, method, url, req_body=body
                    )
                    # Attack
                    ...
                    pkg = self._build_pkg(
                        ev,
                        title=f"Price Manipulation — {field_path}",
                        endpoint=url,
                        vuln_type="Price Manipulation",
                        confidence=85 if confirmed else 60,
                        confirmed=confirmed,
                    )
                    ...
                    f = Finding(
                        ...
                        recommendation=(
                            "Compute all prices server-side from a trusted catalog. "
                            "Never accept client-submitted price values."
                        ),
                        cwe="CWE-20", cvss=9.1,
                        owasp="API6:2023 Unrestricted Access to Sensitive Business Flows",
                        confirmed=confirmed,
                        endpoint=url, parameter=field_path,
                    )
                    self._attach(f, pkg)
    ...
    async def _check_quantity_negative(
        ...
        for key in qty_keys:
            ...
            for tampered in [-1, -100, -9999, 0, -0.5]:
                ...
                # FP canary: server must reject obviously invalid values
                ...
                pkg = self._build_pkg(
                    ev,
                    title=f"Negative Quantity — {key}={tampered}",
                    endpoint=url,
                    vuln_type="Negative Quantity",
                    confidence=80,
                )
                ...
                f = Finding(
                    ...
                    description=(
                        f"`{key}` = {tampered} (original: {original}) accepted at `{url}`. "
                        "May reverse charges or grant negative inventory."
                    ),
                    request={"method": method, "url": url, "body": test_body},
                    response_summary=f"HTTP {status} — {resp_body[:300]}",
                    evidence=(
                        f"qty={tampered} → HTTP {status} "
                        f"(canary 'INVALID_QTY_CANARY' → {cs} ✓)"
                    ),
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # MODULE 3 — IDOR / BOLA (full evidence capture)
    # ═══════════════════════════════════════════════════════════════════════════
    async def _check_idor_bola(
        ...
        for seg_idx, segment in enumerate(path_segments):
            ...
            for test_id in [original_id - 1, original_id + 1, 1, 2, 3]:
                ...
                pkg = self._build_pkg(
                    ev,
                    title=f"IDOR — Path ID {original_id}→{test_id}",
                    endpoint=test_url,
                    vuln_type="IDOR/BOLA",
                    confidence=91 if confirmed else 75,
                    confirmed=confirmed,
                )
                ...
                f = Finding(
                    ...
                    description=(
                        ...
                        f"{'Cross-user access CONFIRMED.' if confirmed else ''}"
                    ),
                    ...
                    recommendation=(
                        ...
                        "Tie resource access to authenticated session, not just the ID."
                    ),
                    cwe="CWE-639",
        ...
        # ── No-auth access on the original URL ────────────────────────────────
        if self.config.no_auth_check:
            ...
            if s_na in (200, 201) and self._response_indicates_success(rb_na, s_na):
                ...
                if sim > 0.7:
                    ...
                    f = Finding(
                        ...
                        description=(
                            f"Removing the Authorization header at `{url}` returned "
                            f"HTTP {s_na} with {sim:.0%} similarity to the authenticated response."
                        ),
                        request={"method": method, "url": url, "note": "No Authorization"},
                        response_summary=f"HTTP {s_na} — {rb_na[:300]}",
                        evidence=(
                            f"No-auth → HTTP {s_na}, "
                            f"similarity to authed response = {sim:.0%}"
                        ),
                        recommendation=(
    ...
    async def _check_bopla(
        ...
        for probe in [
            ...
            # Re-do with probe params (can't pass params through _req_ev easily)
            ...
            pkg = self._build_pkg(
                ev,
                title=f"BOPLA — {list(probe.keys())[0]} param",
                endpoint=url,
                vuln_type="BOPLA",
            ...
            f = Finding(
                ...
                description=(
                    ...
                    f"{'Sensitive fields present!' if has_sensitive else ''}"
                ),
                request={"method": "GET", "url": url, "params": {**params, **probe}},
                response_summary=f"HTTP {status}, {len(resp_body)} vs {base_len} bytes",
                evidence=(
                    f"+{len(resp_body)-base_len} bytes, "
                    f"new keys: {list(new_keys)[:5]}"
                ),
                recommendation=(
                    "Define explicit field allowlists per role. "
                    "Never expose internal fields based on request params."
                ),
                cwe="CWE-213",
    ...
    async def _check_workflow_bypass(
        ...
        for pattern, label in [
            ...
            try:
                for delta in [2, 3]:
                    ...
                    pkg = self._build_pkg(
                        ev,
                        title=f"Workflow Bypass — {label} {current}→{next_step}",
                        endpoint=new_url,
                    ...
                    f = Finding(
                        title=(
                            f"Workflow Step Bypass — {label} "
                            f"{current}→{next_step}"
                        ),
                        severity=Severity.HIGH,
                        category="Business Logic — Workflow Bypass",
                        description=(
                            f"Step {next_step} at `{new_url}` accessible without "
                            f"completing step {current}."
                        ),
                        ...
                        recommendation=(
                            "Enforce sequential step validation using "
                            "signed server-side session state."
                        ),
        ...
        for key in ["otp", "mfa_token", "verification_token"]:
            ...
            if status in (200, 201) and self._response_indicates_success(
                resp_body, status
            ):
                pkg = self._build_pkg(
                    ev,
                    title=f"MFA/OTP Omission — {key}",
                    endpoint=url,
                    vuln_type="MFA Bypass",
                    confidence=75,
                )
                f = Finding(
                    ...
                    category="Business Logic — Workflow Bypass",
                    description=(
                        f"Removing `{key}` from the request at `{url}` "
                        "returned success — MFA/OTP step not enforced."
                    ),
                    request={"method": method, "url": url, "body": test_body},
                    response_summary=f"HTTP {status}",
                    evidence=f"Missing `{key}` → HTTP {status}",
                    recommendation=(
                        "Validate OTP/MFA tokens server-side. "
                        "Require and verify before any action."
                    ),
    ...
    async def _check_mass_assignment(
        ...
        for key in privileged_keys:
            ...
            # FP: confirm baseline doesn't already have this set
            base_data = self._try_parse_json(base_body)
            ...
            f = Finding(
                ...
                description=(
                    f"Injecting `{key}: true` at `{url}` reflected as "
                    f"`{reflected}`. Privilege escalation confirmed."
                ),
                request={"method": method, "url": url, "body": test_body},
                ...
                recommendation=(
                    ...
                    "Never pass raw request bodies to ORM methods."
                ),
        ...
        for admin_path in admin_paths:
            ...
            for token, label in [
                ...
                f = Finding(
                    ...
                    response_summary=f"HTTP {status} — {resp_body[:300]}",
                    evidence=(
                        f"GET {test_url} with {label} "
                        f"→ {status} + {len(resp_body)}B"
                    ),
                    recommendation=(
                        "Enforce RBAC on all admin endpoints using middleware. "
                        "Validate role before executing handler logic."
                    ),
    ...
    async def _check_function_level_access(
        ...
        for test_method in ["DELETE", "PUT", "PATCH"]:
            ...
            if status in (200, 201, 204):
                pkg = self._build_pkg(
                    ev,
                    title=f"BFLA — User2 can {test_method} User1 resource",
                    endpoint=url,
                    vuln_type="BFLA",
                    confidence=90,
                    confirmed=True,
                ...
                f = Finding(
                    ...
                    category="Business Logic — Function Level Access Control",
                    description=(
                        f"User 2 performed {test_method} on `{url}` "
                        f"(owned by User 1) → HTTP {status}."
                    ),
                    request={
                        "method": test_method, "url": url,
                        "note": "Second user token",
                    },
                    response_summary=f"HTTP {status} — {resp_body[:300]}",
                    evidence=f"User2 + {test_method} {url} → {status}",
                    recommendation=(
                        "Verify resource ownership on every mutating request."
                    ),
        ...
        for key in coupon_keys:
            ...
            for test_body, label in [
                ...
                f = Finding(
                    ...
                    recommendation=(
                        "Normalize coupon inputs. "
                        "Enforce one-coupon-per-order server-side."
                    ),
                    cwe="CWE-20",
                    cvss=8.5 if confirmed else 6.5,
                    owasp="API6:2023 Unrestricted Access to Sensitive Business Flows",
        ...
        for key in time_keys:
            ...
            for ts, label in [
                ...
                if (
                    ...
                    pkg = self._build_pkg(
                        ev,
                        title=f"Time Bypass — {key} ({label})",
                        endpoint=url,
                        vuln_type="Time Logic Bypass",
                        confidence=65,
                    )
                    ...
                    f = Finding(
                        ...
                        description=(
                            ...
                            "with a different response from baseline."
                        ),
                        request={"method": method, "url": url, "body": test_body},
                        response_summary=f"HTTP {status}",
                        evidence=f"`{key}` = {ts} → different response from baseline",
                        recommendation=(
                            "Validate all timestamps server-side using server time only."
                        ),
                        cwe="CWE-20", cvss=7.3,
                        owasp="API6:2023 Unrestricted Access to Sensitive Business Flows",
                        endpoint=url, parameter=key,
            ...
            for val in [2**31 - 1, 2**63 - 1, -2**31, 9999999999]:
                ...
                try:
                    ...
                    if status == 500:
                        pkg = self._build_pkg(
                            ev,
                            title=f"Integer Overflow — {key}={val}",
                            endpoint=url,
                        ...
                        f = Finding(
                            ...
                            recommendation="Validate numeric ranges. Handle overflow.",
                            ...
                            for rk, rv in data.items():
                                if (
                                    ...
                                    f = Finding(
                                        ...
                                        category="Business Logic — Integer Overflow",
                                        description=(
                                            f"`{key}`={val} → `{rk}`={rv} (negative) at `{url}`."
                                        ),
                                        request={
                                            "method": method, "url": url,
                                            "body": test_body,
                                        ...
                                        recommendation=(
                                            "Use 64-bit integers. Validate input ranges."
                                        ),
    ...
    async def _check_hidden_parameter_disclosure(
        ...
        for param, val in probes.items():
            ...
            f = Finding(
                ...
                recommendation=(
                    "Remove debug params from production. "
                    "Control verbosity server-side."
                ),
    ...
    async def _check_state_machine_abuse(
        ...
        for key, targets in state_map.items():
            ...
            for state in targets:
                ...
                pkg = self._build_pkg(
                    ev,
                    title=f"State Machine Abuse — {key}→{state}",
                    endpoint=url,
                    vuln_type="State Machine Abuse",
                    confidence=90,
                    confirmed=True,
                )
                ...
                f = Finding(
                    ...
                    description=(
                        f"Forcing `{key}`→`{state}` at `{url}` accepted "
                        "and confirmed in response. Workflow conditions bypassed."
                    ),
                    request={"method": method, "url": url, "body": test_body},
                    ...
                    recommendation=(
                        "Compute state transitions server-side. "
                        "Never trust client state values."
                    ),
        ...
        if success_count > 1:
            ...
            f = Finding(
                ...
                description=(
                    ...
                    "Double-spending or duplicate redemption possible."
                ),
                request={
                ...
                evidence=(
                    f"Concurrent: {success_count}/15 succeeded. "
    ...
    async def _check_jwt_manipulation(
        ...
        # alg:none bypass
        forged_token = f"{b64e({**header, 'alg': 'none'})}.{parts[1]}."
        ev = self._new_evidence()
        await self._req_ev("baseline", ev, method, url, req_body=body if body else None)
        status, _, resp_body, _ = await self._req_ev(
            "attack", ev, method, url,
            req_body=body if body else None,
            token_override=forged_token,
        )

        if status in (200, 201) and self._response_indicates_success(resp_body, status):
            # FP: confirm totally random token is NOT accepted
            ...
            if ts2 not in (200, 201):
                ...
                f = Finding(
                    ...
                    description=(
                        ...
                        "(no signature). Claims can be forged."
                    ),
                    request={
                        "method": method, "url": url,
                        "note": "JWT with alg:none",
                    },
                    response_summary=f"HTTP {status}",
                    evidence=(
    ...
    async def _check_account_enumeration(
        ...
        if (
            ...
            f = self._finalize_finding(
                f, base_body, ev_results[0][2],
                base_status, ev_results[0][1],
            )
            if f.confidence >= self.config.min_confidence:
                findings.append(f)
        ...
        if timing_delta > 0.20:
            f = Finding(
                title=(
                    f"Account Enumeration — timing oracle "
                    f"({timing_delta:.2f}s delta)"
                ),
                severity=Severity.LOW,
                category="Business Logic — Information Disclosure",
                description=(
                    f"Timing delta of {timing_delta:.2f}s between valid/invalid "
                    f"`{field}` values at `{url}`."
                ),
    ...
    async def _check_limit_offset_manipulation(
        ...
        for probe in [
            ...
            pkg = self._build_pkg(
                ev,
                title=f"Limit Manipulation — {probe}",
                endpoint=url,
    ...
    async def _check_soft_delete_bypass(
        ...
        for probe in [
            ...
            pkg = self._build_pkg(
                ev,
                title=f"Soft Delete Bypass — {probe}",
                endpoint=url,
                vuln_type="Soft Delete Bypass",
                confidence=65,
            )
            ...
            f = Finding(
                ...
                description=(
                    ...
                    "deleted records possibly exposed."
                ),
                request={"method": "GET", "url": url, "params": {**params, **probe}},
                response_summary=f"HTTP {status}, {len(resp_body)} bytes",
                evidence=f"Response changed with {probe}",
                recommendation=(
                    "Filter soft-deleted records at the ORM layer. "
                    "Never trust client-supplied visibility flags."
                ),
    ...
    async def _check_http_method_override(
        ...
        for oh in [
            ...
            pkg = self._build_pkg(
                ev,
                title=f"HTTP Method Override — {list(oh.keys())[0]}",
                endpoint=url,
                vuln_type="Method Override",
                confidence=55,
            )
            ...
            f = Finding(
                ...
                description=(
                    f"Override header `{oh}` at `{url}` produced a "
                    "different response from baseline POST."
                ),
    ...
    async def _check_parameter_pollution(
        ...
        for key, val in list(params.items())[:3]:
            ...
            pkg = self._build_pkg(
                ev,
                title=f"Parameter Pollution — {key}",
                endpoint=url,
            ...
            f = Finding(
                ...
                description=(
                    f"Duplicating `{key}` with injected value at `{url}` "
                    "changed the response significantly."
                ),
                request={"method": "GET", "url": test_url},
                response_summary=f"HTTP {status}",
                evidence=f"Duplicate {key} → significantly different response",
                recommendation=(
                    "Take only the first value for duplicate parameters."
                ),
                cwe="CWE-20", cvss=5.8,
                owasp="API6:2023 Unrestricted Access to Sensitive Business Flows",
                endpoint=url, parameter=key,
        ...
        for r in path_results + body_results + header_results:
            ...
            f = Finding(
                ...
                description=(
                    ...
                    f"{'Cross-user CONFIRMED.' if r.confirmed else 'Heuristic.'}"
                ),
        ...
        for gql_url in [
            ...
            if status == 200 and "__schema" in resp_body:
                ...
                f = Finding(
                    ...
                    description=(
                        f"GraphQL introspection at `{gql_url}` exposes the full API schema."
                    ),
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # Phase 1 — Flow attacks + OAuth tests
    # ═══════════════════════════════════════════════════════════════════════════
    async def _run_flow_attacks(self) -> list[Finding]:
        ...
        for fc in flow_configs:
            try:
                ...
                flow_findings = await self._flow_replayer.attack(
    ...
    async def _auto_run_ecommerce_flow(
        self, findings: list[Finding], base_url: str
    ):
