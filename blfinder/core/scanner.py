"""
BLFinder v3.1 Phase 3 — core/scanner.py
Full scanner with all phases integrated:
...
Every _check_* module:
  1. Gates on endpoint validation (skip soft-404, WAF blocks, non-GraphQL)
  2. Records baseline + attack HTTP via EvidenceCapture
  3. Builds EvidencePackage with diff + impact analysis
  4. Attaches package to Finding (f.evidence_package)
  5. Has confidence capped by response classifier
"""
...
class BLFScanner:
    ...
    async def run_all_modules(self, endpoints: list[dict]) -> list[Finding]:
        print(f"[*] BLFinder v3.1 Phase 3 — Target: {self.config.target_url}")
        ...
        # Phase 3: Inject subdomain-discovered endpoints
    ...
    async def _run_endpoint_checks(
        ...
        # Phase 3: Validate endpoint before testing
        is_graphql = any(
            seg in url.lower()
            for seg in ["/graphql", "/gql", "/query"]
        )
        ...
        # Phase 3: Classify the baseline response
        confidence_cap = 100
        ...
        # Phase 3: Apply confidence cap from response classifier
        for f in raw_findings:
            if confidence_cap < 100:
                original  = getattr(f, "confidence", 0)
                capped    = min(original, confidence_cap)
                f.confidence = capped
                if capped < original:
                    fp = getattr(f, "false_positive_checks", [])
                    cr = self._classified_responses.get(url)
                    cr_name = cr.response_class.value if cr else "UNKNOWN"
                    fp.append(
                        f"Confidence capped to {confidence_cap}% "
                        f"by response classifier ({cr_name})"
                    )
                    f.false_positive_checks = fp
            findings.append(f)
        ...
        for key in qty_keys:
            ...
            for tampered in [-1, -100, -9999, 0, -0.5]:
                ...
                pkg = self._build_pkg(
                    ...
                    endpoint=url, vuln_type="Negative Quantity",
                    confidence=80,
                )
                f = Finding(
    ...
    async def _check_idor_bola(
        ...
        for seg_idx, segment in enumerate(path_segments):
            ...
            for test_id in [original_id - 1, original_id + 1, 1, 2, 3]:
                ...
                status, _, resp_body, _ = await self._req_ev(
                    "attack", ev, method, test_url, req_body=body if body else None
                )
                if status != 200:
                    continue
        ...
        # No-auth check on original URL
        if self.config.no_auth_check:
            ev_na = self._new_evidence()
            await self._req_ev(
                "baseline", ev_na, method, url, req_body=body if body else None
            )
                ...
                if sim > 0.7:
                    ...
                    f = Finding(
                        ...
                        description=(
                            ...
                            f"HTTP {s_na} with {sim:.0%} similarity to authenticated response."
                        ),
        ...
        for key in ["otp", "mfa_token", "verification_token"]:
            ...
            if status in (200, 201) and self._response_indicates_success(resp_body, status):
                ...
                f = Finding(
                    ...
                    severity=Severity.HIGH,
                    category="Business Logic — Workflow Bypass",
                    description=f"Removing `{key}` at `{url}` returned success — MFA/OTP not enforced.",
                    request={"method": method, "url": url, "body": test_body},
                    response_summary=f"HTTP {status}",
                        ...
                        if isinstance(data, dict):
                            for rk, rv in data.items():
                                if (
                                    ...
                                    pkg = self._build_pkg(
                                        ev, title=f"Integer Overflow — {key}={val} → {rk}<0",
    ...
    async def _check_jwt_manipulation(
        ...
        status, _, resp_body, _ = await self._req_ev(
            ...
            req_body=body if body else None, token_override=forged,
        )
        if status in (200, 201) and self._response_indicates_success(resp_body, status):
            random_token = "eyJhbGciOiJub25lIn0.eyJ1c2VyIjoiZmFrZSJ9."
            ts2, _, _, _ = await self._request(
                method, url,
                json=body if body else None, token_override=random_token,
            )
            if ts2 not in (200, 201):
                ...
                f = Finding(
                    ...
                    description=f"Server at `{url}` accepted JWT with alg:none (no signature).",
    ...
    async def _check_parameter_pollution(
        ...
        for key, val in list(params.items())[:3]:
            ...
            f = Finding(
                ...
                description=f"Duplicating `{key}` at `{url}` changed the response significantly.",
    ...
    async def _check_graphql_idor(self) -> list[Finding]:
        ...
        for gql_url in [
            ...
            # Phase 3: Validate GraphQL endpoint first
            if _HAS_VALIDATION and self._endpoint_validator:
            ...
            if status == 200 and "__schema" in resp_body:
                # Phase 3: only flag if response is actually JSON GraphQL
                if '"data"' in resp_body or '"errors"' in resp_body:
                    pkg = self._build_pkg(
                    ...
                    f = Finding(
                        ...
                        description=f"GraphQL introspection at `{gql_url}` exposes the full API schema.",
            ...
            for q in [
                ...
                # Phase 3: Must return JSON with "data" key — not HTML
                if (
                    status == 200
                    ...
                    f = Finding(
                        ...
                        response_summary=f"HTTP {status} — {resp_body[:300]}",
                        evidence="Unauthenticated query returned JSON GraphQL data (not HTML)",
    ...
    async def _run_flow_attacks(self) -> list[Finding]:
        ...
        for fc in flow_configs:
            try:
                ...
                findings.extend(
                    await self._flow_replayer.attack_workflow_bypass(steps, base_url=base_url)
                )
