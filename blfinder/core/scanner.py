...
Complete scanner with all phases integrated.
...
Phase 5: Live dashboard integration, profile system, DB deduplication,
         findings queue for TUI consumption, deep discovery pipeline wiring

Integration notes (v3.1 Phase 5):
  - apply_profile() now handles discovery sub-keys and all profile flags
  - run_all_modules() filters already-seen endpoints via DB (--no-repeat)
  - _run_endpoint_checks() respects blind_idor config flag
  - Dashboard state wired to finding queue consumer
  - Deep discovery endpoint merge uses consistent (url, method) key
  - no_scan path correctly skips scanner init while still saving endpoints
  - Profile priority_modules re-order the check list at runtime
  - All ImportError guards use module-level _HAS_* booleans (no silent skips)
"""
...
try:
    from .storage.scan_database import ScanDatabase
    _HAS_DB = True
except ImportError:
    _HAS_DB = False
...
# Canonical check name order — used by priority_modules profile key
_ALL_CHECK_NAMES = [
    "price_manipulation",
    "negative_quantity",
    "workflow_bypass",
...
class BLFScanner:
    ...
    def __init__(self, config: ScanConfig):
        ...
        # Endpoints already confirmed vulnerable in DB (for --no-repeat)
        self._db_seen_endpoints:        set[str]   = set()
    ...
    async def __aenter__(self):
        ...
        # Phase 1
        if _HAS_BLIND_IDOR and getattr(self.config, "blind_idor", True):
        ...
        # Phase 5: pre-load DB-seen endpoints for --no-repeat
        if _HAS_DB and getattr(self.config, "no_repeat", False):
            await self._load_db_seen_endpoints()

        # Phase 5: findings queue (dashboard + DB consumer)
    ...
    # ── Phase 5: DB dedup pre-load ────────────────────────────────────────────

    async def _load_db_seen_endpoints(self):
        """
        Pre-load the set of endpoint URLs that already have confirmed findings
        in the database. Used by --no-repeat to skip re-scanning known-vuln
        endpoints and avoid duplicate entries.
        """
        db_path = getattr(self.config, "db_path", "~/.blfinder.db")
        program = getattr(self.config, "program", "")
        try:
            async with ScanDatabase(db_path) as db:
                findings = await db.get_findings(program=program, status="confirmed")
                self._db_seen_endpoints = {
                    f.endpoint for f in findings if f.endpoint
                }
            if self._db_seen_endpoints:
                print(
                    f"[*] DB dedup: skipping {len(self._db_seen_endpoints)} "
                    f"already-confirmed endpoint(s)"
                )
        except Exception as e:
            if self.config.verbose:
                print(f"  [!] DB dedup pre-load failed: {e}")

    # ── Phase 5: Profile + Dashboard ─────────────────────────────────────────

    def apply_profile(self, profile: dict):
        """
        Apply scan profile settings to the live scanner instance.

        CLI flags already win via merge_profile_with_args() in the entry point,
        so this method only touches keys that were NOT overridden on the CLI.

        Handles:
          settings.*          — rate, depth, confidence, confidence cap, etc.
          skip_modules        — module names to exclude from every endpoint check
          priority_modules    — re-order check list so high-value checks run first
          discovery.*         — deep discovery layer toggles mirrored onto config
          flows               — profile-specified flow templates (already resolved
                                by blfinder.py; stored here for reference only)
        """
        settings = profile.get("settings", {})

        # Rate limiter
        if "rate_limit" in settings:
            self.rate_limiter.base_delay = float(settings["rate_limit"])

        # ScanConfig numeric knobs (only if not already overridden by CLI)
        for config_key in ("fuzz_depth", "min_confidence", "confirmation_attempts"):
            if config_key in settings:
                setattr(self.config, config_key, settings[config_key])

        # Boolean feature flags  ← new: covers all profile-settable flags
        bool_flag_map = {
            "strict_validation":   "strict_validation",
            "blind_idor":          "blind_idor",
            "run_graphql_deep":    "run_graphql_deep",
            "run_websocket":       "run_websocket",
            "run_version_scan":    "run_version_scan",
            "run_recon":           "run_recon",
            "js_secrets":          "run_js_extract",
            "deep_discovery":      "deep_discovery",
            "no_wordlist":         "no_wordlist",
            "no_js_ast":           "no_js_ast",
            "no_openapi":          "no_openapi",
            "no_robots":           "no_robots",
            "no_version_permute":  "no_version_permute",
            "idor_harvest":        "idor_harvest",
            "idor_cross_endpoint": "idor_cross_endpoint",
        }
        for profile_key, config_attr in bool_flag_map.items():
            if profile_key in settings and settings[profile_key]:
                if not getattr(self.config, config_attr, False):
                    setattr(self.config, config_attr, True)

        # Numeric range settings not covered above
        for config_key in ("idor_range", "idor_batch_size", "subdomain_wordlist_size",
                           "wordlist_depth", "ws_race_count"):
            if config_key in settings:
                if not getattr(self.config, config_key, 0):
                    setattr(self.config, config_key, settings[config_key])

        # Discovery sub-dict  ← new
        disc = profile.get("discovery", {})
        if disc:
            disc_cfg = getattr(self.config, "discovery_cfg", None)
            if disc_cfg is not None:
                for attr in ("max_js_files", "wordlist_depth", "business_tags"):
                    if attr in disc and not getattr(disc_cfg, attr, None):
                        try:
                            setattr(disc_cfg, attr, disc[attr])
                        except Exception:
                            pass

        # Module ordering / exclusion
        self._profile_skip_modules     = set(profile.get("skip_modules", []))
        self._profile_priority_modules = profile.get("priority_modules", [])

        # Re-init blind IDOR scanner if profile enables it but __aenter__ skipped it
        if (
            settings.get("blind_idor")
            and _HAS_BLIND_IDOR
            and self._blind_idor is None
        ):
            self._blind_idor = BlindIDORScanner(self)

        # Re-init phase-4 modules enabled by profile after __aenter__
        if settings.get("run_websocket") and _HAS_WEBSOCKET and self._ws_scanner is None:
            self._ws_scanner = WebSocketScanner(self)
        if settings.get("run_version_scan") and _HAS_VERSION and self._version_scanner is None:
            self._version_scanner = APIVersionScanner(self)

        # Forward any unknown settings directly onto config (future-proof)
        known_settings = set(bool_flag_map) | {
            "rate_limit", "fuzz_depth", "min_confidence", "confirmation_attempts",
            "idor_range", "idor_batch_size", "subdomain_wordlist_size",
            "wordlist_depth", "ws_race_count",
        }
        for key, val in settings.items():
            if key not in known_settings and not hasattr(self.config, key):
                setattr(self.config, key, val)

    def set_dashboard_state(self, state: "DashboardState"):
        """Connect the live dashboard state for real-time updates."""
    ...
    def _update_dashboard(
        ...
        """Push a live update to the dashboard state object."""
    ...
    # ── Phase 5: Ordered check list builder ──────────────────────────────────

    def _build_check_list(
        self,
        url: str, method: str, body: dict, params: dict,
        base_status: int, base_body: str, headers: dict,
        skip_set: set[str],
    ) -> list:
        """
        Build the coroutine list for a single endpoint, applying:
          1. profile skip_modules  (already in skip_set via caller)
          2. classifier skip_modules
          3. profile priority_modules  — checked-first re-ordering
          4. blind_idor config flag    — gate blind_idor module
        Returns a list of (name, coro) pairs in execution order.
        """
        # Gate blind_idor on config flag (CLI --blind-idor or profile)
        effective_skip = set(skip_set)
        if not getattr(self.config, "blind_idor", False):
            effective_skip.add("blind_idor")

        all_checks: list[tuple[str, Any]] = [
            ("price_manipulation",   self._check_price_manipulation(url, method, body, params, base_status, base_body)),
            ("negative_quantity",    self._check_quantity_negative(url, method, body, params, base_status, base_body)),
            ("workflow_bypass",      self._check_workflow_bypass(url, method, body, params, base_status, base_body)),
            ("mass_assignment",      self._check_mass_assignment(url, method, body, params, base_status, base_body)),
            ("idor_bola",            self._check_idor_bola(url, method, body, params, base_status, base_body, headers)),
            ("bopla",                self._check_bopla(url, method, body, params, base_status, base_body)),
            ("privilege_escalation", self._check_privilege_escalation(url, method, body, params, base_status, base_body)),
            ("bfla",                 self._check_function_level_access(url, method, body, params, base_status, base_body)),
            ("coupon_stacking",      self._check_coupon_stacking(url, method, body, params, base_status, base_body)),
            ("time_bypass",          self._check_time_logic_bypass(url, method, body, params, base_status, base_body)),
            ("integer_overflow",     self._check_integer_overflow(url, method, body, params, base_status, base_body)),
            ("hidden_parameter",     self._check_hidden_parameter_disclosure(url, method, body, params, base_status, base_body)),
            ("state_machine_abuse",  self._check_state_machine_abuse(url, method, body, params, base_status, base_body)),
            ("race_condition",       self._check_race_condition(url, method, body, params, base_status, base_body)),
            ("jwt_manipulation",     self._check_jwt_manipulation(url, method, body, params, base_status, base_body, headers)),
            ("account_enumeration",  self._check_account_enumeration(url, method, body, params, base_status, base_body)),
            ("limit_offset",         self._check_limit_offset_manipulation(url, method, body, params, base_status, base_body)),
            ("soft_delete_bypass",   self._check_soft_delete_bypass(url, method, body, params, base_status, base_body)),
            ("http_method_override", self._check_http_method_override(url, method, body, params, base_status, base_body)),
            ("parameter_pollution",  self._check_parameter_pollution(url, method, body, params, base_status, base_body)),
            ("blind_idor",           self._check_blind_idor(url, method, body, params, base_status, base_body)),
        ]

        # Build index for priority re-ordering
        check_map   = {name: coro for name, coro in all_checks}
        ordered     = list(_ALL_CHECK_NAMES)  # default order

        if self._profile_priority_modules:
            front = [n for n in self._profile_priority_modules if n in check_map]
            rest  = [n for n in ordered if n not in set(front)]
            ordered = front + rest

        return [
            (name, check_map[name])
            for name in ordered
            if name in check_map and name not in effective_skip
        ]
    ...
    async def _run_phase4_scans(self, endpoints: list[dict]) -> list[Finding]:
        ...
        if _HAS_GQL_DEEP and self._gql_deep and getattr(
            ...
            for gql_path in ["/graphql", "/api/graphql", "/query"]:
                ...
                try:
                    ...
                    if self.config.verbose and result.findings:
                        print(
                            f"  [+] GraphQL deep: {len(result.findings)} findings "
                            f"on {gql_url}"
                        )
                except Exception as e:
                    if self.config.verbose:
        ...
        if _HAS_WEBSOCKET and self._ws_scanner:
            try:
                ...
                for ep in ws_eps[:5]:
                    ...
                    if self.config.verbose and result.findings:
                        print(
                            f"  [+] WebSocket: {len(result.findings)} findings "
                            f"on {ep.url}"
                        )
            except Exception as e:
                if self.config.verbose:
        ...
        if _HAS_VERSION and self._version_scanner:
            try:
                ...
                if self.config.verbose and result.all_findings:
                    print(f"  [+] Version scan: {len(result.all_findings)} findings")
            except Exception as e:
                if self.config.verbose:
    ...
    async def run_all_modules(self, endpoints: list[dict]) -> list[Finding]:
        ...
        # ── Phase 5: DB dedup — filter already-seen endpoints ─────────────────
        if self._db_seen_endpoints:
            pre_filter = len(endpoints)
            endpoints = [
                ep for ep in endpoints
                if ep.get("url", "") not in self._db_seen_endpoints
            ]
            skipped_by_db = pre_filter - len(endpoints)
            if skipped_by_db:
                print(f"[*] --no-repeat: skipped {skipped_by_db} already-confirmed endpoint(s)\n")

        # Dashboard: set total after dedup
        ...
        if self.config.smart_discovery:
            ...
            for ep in extra:
                ...
                if key not in existing:
                    # Respect --no-repeat for discovered endpoints too
                    if ep["url"] not in self._db_seen_endpoints:
                        endpoints.append(ep)
                        existing.add(key)
        ...
        if self._subdomain_map:
            ...
            for ep in sub_eps:
                key = (ep["url"], ep.get("method", "GET"))
                if key not in existing and ep["url"] not in self._db_seen_endpoints:
                    endpoints.append(ep)
                    existing.add(key)
                    added += 1
            if added:
        ...
        # Phase 4: Build attack plan + classify
        ...
        # Standard endpoint scan — tasks keyed by resolved URL
        tasks = []
        for ep in endpoints:
            url = ep.get("url", "")
        ...
        # Push all findings to queue for DB / TUI consumers
    ...
    async def _run_endpoint_checks(
        self, url: str, method: str, body: dict, params: dict
    ) -> list[Finding]:
        findings: list[Finding] = []
        ...
        # Phase 4: Harvest IDs for IDOR mass enumeration
        if _HAS_IDOR_ENUM and self._idor_enumerator:
        ...
        # Phase 4+5: Combined skip set (classifier + profile)
        ...
        # Build ordered check list respecting skip + priority + blind_idor flag
        named_checks = self._build_check_list(
            url, method, body, params, status, base_body, headers, skip_set
        )
        checks_to_run = [coro for _, coro in named_checks]
        ...
        # Apply confidence cap + dashboard + queue
    ...
    async def _check_price_manipulation(
        self, url, method, body, params, base_status, base_body
    ) -> list[Finding]:
        findings: list[Finding] = []
        price_keys = [
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    async def _check_quantity_negative(
        self, url, method, body, params, base_status, base_body
    ) -> list[Finding]:
        findings: list[Finding] = []
        qty_keys = [
            "quantity", "qty", "count", "units",
            "items", "amount", "number", "stock",
        ]
        for key in qty_keys:
            if key not in body:
                continue
            ...
            for tampered in [-1, -100, -9999, 0, -0.5]:
                ...
                if cs in (200, 201):
                    continue
                pkg = self._build_pkg(
                    ev, title=f"Negative Quantity — {key}={tampered}",
                ...
                f = Finding(
                    ...
                    description=(
                        f"`{key}` = {tampered} (original: {original}) "
                        f"accepted at `{url}`."
                    ),
    ...
    # MODULE 3 — IDOR / BOLA
    # ═══════════════════════════════════════════════════════════════════════════
    async def _check_idor_bola(
        self, url, method, body, params, base_status, base_body, resp_headers
    ) -> list[Finding]:
        findings: list[Finding] = []
        parsed        = urlparse(url)
        ...
        for seg_idx, segment in enumerate(path_segments):
            ...
            for test_id in [original_id - 1, original_id + 1, 1, 2, 3]:
                ...
                if not self._responses_differ_significantly(base_body, resp_body):
                    continue
                if any(
                    t in resp_body.lower()
                    for t in ["not found", "no resource", "does not exist"]
                ):
                    continue
                confirmed = False
                ...
                f = Finding(
                    title=(
                        f"IDOR/BOLA — Path ID {original_id}→{test_id} "
                        "returned different resource"
                    ),
                    ...
                    confirmed=confirmed, endpoint=test_url,
                    parameter=f"path[{seg_idx}]",
                )
                self._attach(f, pkg)
                f = self._finalize_finding(f, base_body, resp_body, base_status, status)
                if f.confidence >= self.config.min_confidence:
        ...
        if self.config.no_auth_check:
            ...
            await self._req_ev(
                "baseline", ev_na, method, url,
                req_body=body if body else None,
            )
            s_na, _, rb_na, _ = await self._req_ev(
                "attack", ev_na, method, url,
                req_body=body if body else None, token_override="",
            )
            if s_na in (200, 201) and self._response_indicates_success(rb_na, s_na):
                sim = difflib.SequenceMatcher(
                    None, base_body[:2000], rb_na[:2000]
                ).ratio()
                if sim > 0.7:
                    pkg_na = self._build_pkg(
                        ev_na, title="Unauthenticated Access",
        ...
        for probe in [
            ...
            if ev:
                ev.record(
                    ...
                    req_body=None, resp_status=status,
                    resp_headers={}, resp_body=resp_body, elapsed=elapsed,
                )
            ...
            sensitive = [
                ...
                "ssn", "dob", "credit_card", "cvv", "salary", "hash",
            ]
            has_sensitive = any(
                sf in " ".join(new_keys).lower() for sf in sensitive
            )
            pkg = self._build_pkg(
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    async def _check_workflow_bypass(
        self, url, method, body, params, base_status, base_body
    ) -> list[Finding]:
        findings: list[Finding] = []
        parsed = urlparse(url)
        ...
        for key in ["otp", "mfa_token", "verification_token"]:
            ...
            if status in (200, 201) and self._response_indicates_success(resp_body, status):
                ...
                f = Finding(
                    ...
                    category="Business Logic — Workflow Bypass",
                    description=(
                        f"Removing `{key}` at `{url}` returned success."
                    ),
                    request={"method": method, "url": url, "body": test_body},
                    response_summary=f"HTTP {status}",
    ...
    async def _check_mass_assignment(
        ...
        for key in privileged_keys:
            ...
            pkg = self._build_pkg(
                ...
                endpoint=url, vuln_type="Mass Assignment",
                confidence=90, confirmed=True,
            )
            f = Finding(
                ...
                description=(
                    f"Injecting `{key}: true` at `{url}` "
                    f"reflected as `{reflected}`."
                ),
                request={"method": method, "url": url, "body": test_body},
    ...
    async def _check_privilege_escalation(
        ...
        for admin_path in admin_paths:
            ...
            for token, label in [
                (self.config.auth_token, "low-priv"),
                ("", "no-auth"),
            ]:
                ev = self._new_evidence()
                status, _, resp_body, _ = await self._req_ev(
                ...
                pkg = self._build_pkg(
                    ...
                    endpoint=test_url, vuln_type="BFLA",
                    confidence=90, confirmed=True,
                )
                f = Finding(
                    ...
                    response_summary=f"HTTP {status} — {resp_body[:300]}",
                    evidence=(
                        f"GET {test_url} with {label} → {status} + {len(resp_body)}B"
                    ),
    ...
    async def _check_function_level_access(
        self, url, method, body, params, base_status, base_body
    ) -> list[Finding]:
        findings: list[Finding] = []
        ...
        for test_method in ["DELETE", "PUT", "PATCH"]:
            ...
            if status in (200, 201, 204):
                ...
                f = Finding(
                    ...
                    category="Business Logic — Function Level Access Control",
                    description=(
                        f"User 2 performed {test_method} on `{url}` → HTTP {status}."
                    ),
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    async def _check_coupon_stacking(
        self, url, method, body, params, base_status, base_body
    ) -> list[Finding]:
        findings: list[Finding] = []
        coupon_keys = [
            "coupon", "coupon_code", "promo_code",
            "discount_code", "voucher", "promo",
        ]
        for key in coupon_keys:
            ...
            for test_body, label in [
                ...
                f = Finding(
                    ...
                    description=(
                        f"Coupon `{key}` as {label} accepted at `{url}`."
                    ),
                    request={"method": method, "url": url, "body": test_body},
                    response_summary=f"HTTP {status}",
                    evidence=(
                        f"Coupon as {label}: "
                        f"discount {base_discount}→{new_discount}"
                    ),
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    async def _check_time_logic_bypass(
        self, url, method, body, params, base_status, base_body
    ) -> list[Finding]:
        findings: list[Finding] = []
        time_keys = [
            ...
            for ts, label in [
                ...
                if (
                    ...
                    pkg = self._build_pkg(
                        ...
                        endpoint=url, vuln_type="Time Logic Bypass",
                        confidence=65,
                    )
                    f = Finding(
                        ...
                        response_summary=f"HTTP {status}",
                        evidence=(
                            f"`{key}` = {ts} → different response from baseline"
                        ),
                        recommendation=(
                            "Validate all timestamps server-side using server time."
                        ),
                        cwe="CWE-20", cvss=7.3,
                        owasp="API6:2023 Unrestricted Access to Sensitive Business Flows",
                        endpoint=url, parameter=key,
            ...
            for val in [2**31 - 1, 2**63 - 1, -2**31, 9999999999]:
                ...
                try:
                    ...
                    if status == 500:
                        ...
                        self._attach(f, pkg)
                        f = self._finalize_finding(
                            f, base_body, resp_body, base_status, 500
                        )
                        if f.confidence >= self.config.min_confidence:
                            findings.append(f)
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    async def _check_hidden_parameter_disclosure(
        self, url, method, body, params, base_status, base_body
    ) -> list[Finding]:
        findings: list[Finding] = []
        probes = {
            "debug": "1", "verbose": "1", "admin": "1",
            "internal": "1", "full": "1", "include_deleted": "1",
            "show_all": "1", "_debug": "1",
        }
        for param, val in probes.items():
            ...
            pkg = self._build_pkg(
                ...
                endpoint=url, vuln_type="Information Disclosure",
                confidence=65,
            )
            f = Finding(
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    async def _check_state_machine_abuse(
        self, url, method, body, params, base_status, base_body
    ) -> list[Finding]:
        findings: list[Finding] = []
        state_map = {
        ...
        for key, targets in state_map.items():
            ...
            for state in targets:
                ...
                f = Finding(
                    ...
                    description=(
                        f"Forcing `{key}`→`{state}` at `{url}` accepted and confirmed."
                    ),
                    request={"method": method, "url": url, "body": test_body},
                    response_summary=f"HTTP {status}, {key}={state}",
                    evidence=(
                        f"Forced {key}={state} → confirmed (was: {body.get(key)})"
                    ),
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    async def _check_jwt_manipulation(
        self, url, method, body, params, base_status, base_body, resp_headers
    ) -> list[Finding]:
        findings: list[Finding] = []
        ...
        if status in (200, 201) and self._response_indicates_success(resp_body, status):
            ...
            if ts2 not in (200, 201):
                ...
                f = Finding(
                    ...
                    description=(
                        f"Server at `{url}` accepted JWT with alg:none."
                    ),
    ...
    async def _check_account_enumeration(
        ...
        if (
            (
                any(s in r1_body for s in not_found)
                and any(s in r2_body for s in wrong_pass)
            ) or (
                any(s in r2_body for s in not_found)
                and any(s in r1_body for s in wrong_pass)
            )
        ):
            f = Finding(
        ...
        if timing_delta > 0.20:
            f = Finding(
                ...
                category="Business Logic — Information Disclosure",
                description=(
                    f"Timing delta of {timing_delta:.2f}s at `{url}`."
                ),
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    async def _check_limit_offset_manipulation(
        self, url, method, body, params, base_status, base_body
    ) -> list[Finding]:
        findings: list[Finding] = []
        for probe in [
            ...
            pkg = self._build_pkg(
                ...
                endpoint=url, vuln_type="Excessive Data Exposure",
                confidence=70,
            )
            f = Finding(
                title=(
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    async def _check_soft_delete_bypass(
        self, url, method, body, params, base_status, base_body
    ) -> list[Finding]:
        findings: list[Finding] = []
        for probe in [
            ...
            pkg = self._build_pkg(
                ...
                endpoint=url, vuln_type="Soft Delete Bypass",
                confidence=65,
            )
            f = Finding(
                ...
                description=(
                    f"`{probe}` at `{url}` returned a different response."
                ),
                request={"method": "GET", "url": url, "params": {**params, **probe}},
    ...
    async def _check_http_method_override(
        ...
        for oh in [
            ...
            status, _, resp_body, _ = await self._req_ev(
                "attack", ev, "POST", url,
                req_body=body, extra_headers=oh,
            )
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    async def _check_parameter_pollution(
        self, url, method, body, params, base_status, base_body
    ) -> list[Finding]:
        findings: list[Finding] = []
        if not params:
        ...
        for key, val in list(params.items())[:3]:
            ...
            f = Finding(
                ...
                description=(
                    f"Duplicating `{key}` at `{url}` changed the response."
                ),
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # MODULE 20 — Blind IDOR (Phase 1 oracle-based)
    # ═══════════════════════════════════════════════════════════════════════════
    async def _check_blind_idor(
        self, url, method, body, params, base_status, base_body
    ) -> list[Finding]:
        # Guard: module disabled if blind_idor flag is off or scanner not init'd
        ...
        for gql_url in [
            ...
            if (
                ...
                f = Finding(
                    ...
                    description=(
                        f"GraphQL introspection at `{gql_url}` exposes the full schema."
                    ),
            ...
            for q in [
                ...
                if (
                    ...
                    f = Finding(
                        ...
                        category="Business Logic — GraphQL",
                        description=(
                            f"GraphQL query at `{gql_url}` returned data "
